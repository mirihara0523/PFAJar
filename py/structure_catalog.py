...
def compact_ccf_level_label_and_tooltip(info: dict[str, Any]) -> tuple[str, str]:
    """Return compact CCF combo text and its sample-region tooltip."""
    visible = f"Level {info['level']} - {info['count']} {info['kind']}"
    samples = list(info.get("sampleAcronyms") or [])
    if not samples:
        return visible, ""
    detail = ", ".join(samples)
