...
# Cache of tilt-offset grids for slice_3d_volume, keyed by
# (height, width, x_angle, y_angle). Reused across AP changes at a fixed angle.
_TILT_GRID_CACHE = {}
...
def slice_3d_volume(volume, z_position, x_angle, y_angle):
    ...
    # Fast path: with no tilt the slice is just the AP plane. Direct indexing is
    # bit-identical to nearest-neighbor map_coordinates here but avoids building
    # a full coordinate grid and interpolating the whole volume — the main cost
    # when only the AP position changes (the common case).
    if abs(x_angle) < 1e-9 and abs(y_angle) < 1e-9:
        zi = int(round(z_position))
        zi = max(0, min(volume.shape[0] - 1, zi))
        return volume[zi].copy()

    # Tilted slice. The tilt offset grid (x*tan + y*tan) depends only on the
    # angles and volume shape — not on the AP position — so cache it. When many
    # sections share the same angle and only AP changes, this skips rebuilding
    # the meshgrid and tan grid each call. The map_coordinates interpolation
    # itself is unchanged, so the result is identical.
    key = (volume.shape[1], volume.shape[2], float(x_angle), float(y_angle))
    cached = _TILT_GRID_CACHE.get(key)
    if cached is None:
        x, y = np.meshgrid(np.arange(volume.shape[2]), np.arange(volume.shape[1]))
        dz = (
            x * np.tan(np.deg2rad(x_angle)) + y * np.tan(np.deg2rad(y_angle))
        ).astype(np.float32)
        if len(_TILT_GRID_CACHE) > 64:
            _TILT_GRID_CACHE.clear()
        cached = (dz, y, x)
        _TILT_GRID_CACHE[key] = cached
    dz, y, x = cached

    # Adjust z-position based on tilt angles
    z = (z_position + dz).astype(np.float32)
...
def add_outlines(annotation_slice, color_annotation_slice):
    ...
    # Boundary = pixels on ONE side of a label transition (1px thin outline).
    # Vectorized diff (no per-label find_contours loop) — far faster on full-res
    # label arrays. Marking a single side keeps the outline ~1px (matching the
    # previous find_contours look) instead of a thicker 2px both-sides line.
    lab = np.asarray(annotation_slice)
    boundary = np.zeros(lab.shape, dtype=bool)
    boundary[:-1, :] |= lab[:-1, :] != lab[1:, :]
    boundary[:, :-1] |= lab[:, :-1] != lab[:, 1:]

    color_annotation_slice[boundary] = 0
