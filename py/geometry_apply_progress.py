"""Persist geometry apply progress under .masonjar."""
...
def progress_path(bundle_root: Path) -> Path:
    return meta_dir(bundle_root) / "geometry_apply_progress.json"


def last_result_path(bundle_root: Path) -> Path:
    return meta_dir(bundle_root) / "geometry_apply_last_result.json"
